...
def review_obj(ad_obj, user2):
    return Comment.objects.create(
        text="Test Comment", ad=ad_obj, author=user2, created_at=datetime.now()
    )
...
def test_read_review():
    review = Comment.objects.filter(text="Test Comment").first()
...
def test_update_review():
    review = Comment.objects.filter(text="Test Comment").first()
    review.text = "Updated Comment Text"
    review.save()
    updated_review = Comment.objects.filter(text="Updated Comment Text").first()
...
def test_delete_review():
    ...
    assert Comment.objects.filter(text="Test Comment").count() == 0
